...
`claude/21_S8_Subcontractor_Portal_Built.md`. Next in-house item: 8, S1.
...
npm run guard:test                  # 268 unit tests: resolver, admission, gate, sync, SLA, money, D14, uow, auth, session, refusals, hierarchy, contract, network and settlement handlers, catalogue, sdk, shell (incl. the page lifecycle), tokens→CSS, the accent gate, the brand layer, the badge, PR titles
...
npm run test:integration            # 113 tests: the contract with RLS on (29) + the shell and the browser session over the wire (19) + C1 (12) + C2 (11) + C4 (8) + item 4, S3/S5: the gated door, the shift token, the offline replay (10) + item 6, S6: tier scoping as four customer principals see it (10) + item 7, S8: firm visibility as two firms and the office see it, and the four writes (10) + the worker's sweeps bound under RLS, on a scratch database (4)
...
npm run test:ui                     # 105 tests rendered to a string under node --test: packages/ui + S2 (C1, C2, C4), S3, S5, S6 and S8 screens through the real shell against a scripted gateway. Need preact, hence here
...
packages/shell/       S0: createShell (registry checks), connectShell (login → me → shell), subscribe (SSE, dedupe; closed on pagehide, reopened from the back/forward cache), refusalOf, degraded driven by the wire; brand.ts installs a tenant's block BEFORE login — a portal is branded on its sign-in screen
...
test/integration/     backbone.test.ts — the contract against a live Postgres · s0-shell.test.ts — the shell against a spawned gateway · s2-c1 · s2-c2 · s2-c4 · s3-s5 (item 4) · s6 (item 6) · s8 (item 7) · worker (the sweeps under RLS, on a scratch database)
