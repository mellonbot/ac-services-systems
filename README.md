...
Detail: `claude/22_S1_Marketing_Built.md`.

**Item 9 is built (2026-09-21): the S6 SITE CARD** — what a site IS, on one
page, reached from its line on the Sites tree and from any job's site: where
it is (the address as recorded, and overhead imagery fetched BY THE GATEWAY
from a configured provider, so no customer address ever leaves the customer's
browser for a third party and no key is in a bundle — OPEN-S6-IMAGERY); what
runs there (`equipment` gained `kind` and `label`; "last serviced" is DERIVED
from the completed jobs that named the unit through the new `job_equipment`,
written by `jobs.create`'s `equipmentIds`, so it cannot fall behind the work);
who is there (`account_contacts` — the customer's on-site manager and their
number, inherited from the location when the site names none); and one
chronological ledger — requests, jobs and invoice lines at that site, newest
first, money from integer minor units. A **Request service** button sits in the
masthead of every signed-in screen, and the card's own preselects its site.
**Migration 0009** is the item's substance the way 0006 was item 6's. Read as
a facility manager before it: every unit of every customer with its serial
(`equipment` had NO policy), the parent's every invoice nationwide (0002's org
rule), every invoice line of every customer, every photo key, part and warranty
claim (none had a policy). Now a row that hangs off a job is visible exactly
when the job is; a row that hangs off a node exactly when the node is; an
invoice when one of its lines is, and a line when the job it bills (or, with
no job, the location it itemises) is — so a location-scoped manager reads the
consolidated parent invoice with its own lines and its own subtotal beside the
header total. Writing is ours alone, at the table. Six operations
(`equipment.list`/`.register`, `contacts.list`/`.set`, `invoices.list`,
`sites.imagery`); S2 writes, S6 reads. Proven:
`test/integration/s6-site-card.test.ts` (7, including the whole imagery pipe
against a stub provider) and `tools/ci/drive-s6.ts` grown to **15 checks** —
the card in a real browser, full-page, no sideways scroll. **And a defect the
wire test found on a non-UTC cluster:** every `to_char(timestamptz, …"Z")` in
the gateway formatted the SESSION's wall clock with a "Z" glued on, so on a
Chicago cluster every SLA due time, job opening, request time and statement
date on the wire was five hours early. Fixed with `AT TIME ZONE 'UTC'` in
jobs, devices, service requests and settlements, with a regression that
compares a typed ISO instant to the one that comes back. Detail:
`claude/24_S6_Site_Card_Built.md`. Next in-house item: S4 (Phase 2).
...
npm run guard:test                  # 300 unit tests: resolver, admission, gate, sync, SLA, money, D14, uow, auth, session, refusals, hierarchy, contract, network, settlement, lead, site-record and imagery handlers, catalogue, sdk, shell (incl. the page lifecycle), tokens→CSS, the accent gate, the brand layer, the badge, PR titles
...
npm run test:integration            # 136 tests: the contract with RLS on (29) + the shell and the browser session over the wire (19) + C1 (12) + C2 (11) + C4 (8) + item 4, S3/S5: the gated door, the shift token, the offline replay (10) + item 6, S6: tier scoping as four customer principals see it (10) + item 7, S8: firm visibility as two firms and the office see it, and the four writes (10) + item 8, S1: what a stranger's binding reads (nothing) and its two writes (15) + item 9, the S6 site card: units, contacts, invoice lines and imagery as four customer principals see them (7) + the worker's sweeps bound under RLS and on the right calendar day, on a scratch database (5)
...
npm run test:ui                     # 125 tests rendered to a string under node --test: packages/ui + S1, S2 (C1, C2, C4), S3, S5, S6 and S8 screens through the real shell against a scripted gateway. Need preact, hence here
...
#   AC_IMAGERY_URL=https://…?c={lat},{lng}&z={zoom}&s={w}x{h}&key=…  AC_IMAGERY_ATTRIBUTION="© …"   → the site card's overhead imagery, fetched by the gateway (item 9). Unset → the card says so and draws the address alone.
...
apps/gateway/src/handlers/site-record.ts, imagery.ts  item 9: the site record's three reads and two writes; the roof fetched by the gateway
...
test/integration/     backbone.test.ts — the contract against a live Postgres · s0-shell.test.ts — the shell against a spawned gateway · s2-c1 · s2-c2 · s2-c4 · s3-s5 (item 4) · s6 (item 6) · s6-site-card (item 9) · s8 (item 7) · s1 (item 8) · worker (the sweeps under RLS and on the right calendar day, on a scratch database)
...
| 9/S6 | **A site's record is the customer's own — its units, its people, its bill — and nothing beside it** (the S6 site card). Before 0009 the sentence held for nothing the card reads: `equipment`, `invoice_lines`, `job_media`, `parts_used` and `warranty_cases` had NO policy — every unit of every customer with its serial, every line of every invoice, readable from any binding — and `invoices` answered the whole parent's ledger to a manager scoped to one location. None of it through a screen; the fourth migration to say that is not a mechanism | Migration 0009: `ac_node_row_visible(account)` and `ac_job_row_visible(job)` — an EXISTS against the table whose policy already answers the question, so `equipment` and `account_contacts` follow their node, and `job_equipment`, `job_media`, `parts_used`, `warranty_cases` follow their job, for every namespace at once; `ac_invoice_line_visible` (own org, and the job's or the location's rule) and `ac_invoice_visible` (own org, and one visible line); INSERT/UPDATE for the internal namespace alone on every table a customer reads; `ac_job_equipment_is_at_the_jobs_site` holds that a job names units at its own site from every path. "Last serviced" is a LATERAL over the completed jobs in `job_equipment`, never a column; a node's invoice subtotal is the sum of the lines RLS returned. `sites.imagery` is fetched by the gateway from `AC_IMAGERY_URL` — the surface holds no URL and sends no address anywhere but to us. `AccountWire` carries `address`; `CreateJobInput` carries `equipmentIds`; every `*_milli` and `*_minor` on the new wire is a string | 11 handler tests (site-record, imagery — the template, the cache, every provider failure an `unavailable`); 4 render tests (the card, the header button, the helpers' bigint arithmetic); **7 wire tests** — four customer principals: Austin's three units with last-serviced the PM's window end and the OPEN repair not moving it, Reno's unit `unknown_site` to Austin's manager; contacts nearest-first with the location's marked inherited and a replace that adds no row; one two-line parent invoice read as Austin's line and subtotal, Reno's line and subtotal, both for the executive, none for the other customer; the imagery pipe against a stub provider — one fetch, then the cache, `no_coordinates`, `unknown_site`; at the binding Austin's units, one line, zero rows on three job-hung tables, and a forged unit, contact and line edit refused at the table; and the UTC regression. **`tools/ci/drive-s6.ts` — 15 checks in a real browser, run by CI**, the card captured full-page and asserted not to scroll sideways |
