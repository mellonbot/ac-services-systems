...
designed in `09_Surface_Runtime_and_S2_Design.md`) has its first five items
...
`build-surface.ts` as the one build step, five guards), C1: the seven
hierarchy operations in the catalogue with their handlers, and S2's first
screens — login, the tree, new node, move — and C2: the five agreement and
register operations, `handlers/contracts.ts`, and S2's agreements list, the
three-state OQ5 form, the register-driven override form and the resolution
trace. Both flows are driven in a real browser against the real gateway.**
C3 (Amped recorded in full through the screens) waits on OQ1/OQ5; C4 (the
subcontractor network registry, item 7) is next. `docs/BACKBONE_CONTRACT.md` is the
...
npm run test:integration            # 71 tests: the contract with RLS on (29) + the shell and the browser session over the wire (19) + C1 over the wire (12) + C2 over the wire: the two findings as the two axes, OQ5 as a 400, the overlap-then-orphan chain, the ladder (11)

# The toolchain layer.
pnpm install
pnpm typecheck && pnpm guard:lint   # tsc strict; eslint with the eight ac/ rules (verified: they fire)
npm run test:ui                     # 49 tests rendered to a string under node --test: packages/ui + S2's screens (C1 and C2) through the real shell against a scripted gateway. Need preact, hence here
...
apps/gateway/         the sole access path
  ...
  src/handlers/         terms (admission + resolution), assignment (the gate), sync (device replay), hierarchy (C1: org+first region node in one uow, node create/move/update; D14; structure left to the triggers), contracts (C2: the agreement's own shape — scope region, OQ5, the window, the amendment's MSA, the state ladder; scope existence left to the trigger)
